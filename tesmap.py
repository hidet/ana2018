...
spacing = 520# um
rectangle_l = 350# um
g_nrow=30# number of using rows

# ch, position x, y, row number
position=[
    [  1,      290   ,   -3470,    0 ],
    [  3,      290   ,   -4510,    1 ],
    [  5,      290   ,   -1910,    2 ],
...
# assumed sorted by channel number
...
def getDensityColors(resols):# normalized by resolution
    normal = matplotlib.colors.Normalize(resols.min(),resols.max())
...
def calcTwoPixDistance(col1,pix1,col2,pix2):
    ...
    ch1 = 2*(row1+col1*g_nrow)+1
    ch2 = 2*(row2+col2*g_nrow)+1
    x1, y1 = getTESPos(ch1)
    x2, y2 = getTESPos(ch2)
    d = sqrt(pow((x1-x2),2) + pow((y1-y2),2))
    return d
...
def getPlotRectPlane(i=1,j=1,k=1,chans=[1]):
    # return ax to current figure
    l=rectangle_l
    fig=plt.gcf()
    ax =fig.add_subplot(i,j,k, aspect='equal',xlim=(-5000,5000),ylim=(-5000,5000))
    for ch in chans:
        x,y = getTESPos(ch)
