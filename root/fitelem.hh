...
public:
  ...
  TString  GetFuncName() {return funcname_;}
  ...
  TCanvas* Plot(TH1F *h,Bool_t ENE);
  void Print();
...
TCanvas* FitElem::Plot(TH1F *h,Bool_t ENE=false)
{
  Double_t bw=h->GetBinWidth(1);
  TString caname = Form("c_%s_%s",h->GetName(),tag_.Data());
  TCanvas *cfit = new TCanvas(caname,caname,1,1,401,301);
  gPad->SetGrid(0,0);
  gPad->SetTicks();
...
void FitElem::SaveParams(Bool_t ENE=false)
{
  TString foutname=Form("%s/%s/%s_%s",par_dir.Data(),fitElem_tag.Data(),funcname_.Data(),tag_.Data());
...
inline fitparam FitElemXray(TH1F *h1,
                            Int_t nrebin=1,Double_t pp=0,TString ln="CrKAlpha",
  ...
  
{
  ...
  FitElem ff(xl,LET,HET,BG,funcname,xl_tag,low,high,npx);
  ...
  if (PLOT) {
    ...
    if (SAVE) {
      //TString coutname=Form("%s/%s/%s_%s",fig_dir.Data(),fitElem_tag.Data(),funcname_.Data(),tag_.Data());
...
inline fitparam FitElemXrayItr(TH1F *h1,TSpline3 *sp3mean,
                               Int_t nrebin=1,TString ln="CrKAlpha",
                               Bool_t LET=true,Bool_t HET=false,Bool_t BG=false,
                               Bool_t ENE=false,Bool_t PLOT=true,Bool_t SAVE=true)
{
  // h1: pointer of histogram, refer should work
  // nrebin: rebin to fit for the histogram
  // sp3mean: spline3 with fitted means
  // ln: name of the xray line (e.g., MnKAlpha, MnKa, mnka)
  // LET: bool for low-energy tail on/off
  // HET: bool for high-energy tail on/off
  // BG: bool for background function on/off
  ...
  Double_t mean=xl.energies[0];
  if (!ENE) {
    e2c = 1./sp3mean->Derivative(xl.energies[0]);
    mean = sp3mean->Eval(xl.energies[0]);
  }
  Double_t gaussian_sigma = 5;
