""" khe_ana.py describes KHE class used for E62 analysis 
...
2018-08-17 ; ver 1.3; file divided to khe_ext,_group,_dump, T.H.
...
import khe_group as grp
import khe_dump as dump
...
class KHE():
    ...
    def anahide(self,forceNew=False,summaryNew=False,calibNew=False,exttrigNew=False,grptrigNew=False,bcutflag=True):
        ...
        self.timing_analysis(forceNew=exttrigNew)
        ...
        if self.bonly==False:
            ...
            self.mass_analysis_transfer_calibration(bcutflag=bcutflag)
    ...
    def mass_analysis_transfer_calibration(self, bcutflag=True):
        data = self.data
        calh5name = "{}/run{:04d}/run{:04d}_mass.hdf5".format(self.DATADIR,self.calibration_runnum, self.calibration_runnum)
        print "calh5name=",calh5name
        self.caldata = mass.TESGroupHDF5(calh5name)
        if bcutflag:
            bcut = self.get_basic_cuts()
            data.apply_cuts(bcut, forceNew=True)
        #self.data.auto_cuts()
        for ds in self.data:
            print("transfer analysis on {}".format(ds.channum))
    ...
    def dump_ROOT_2018(self, EXTTRIG=False, GRTRIG=False, dumppulse=False):
        dump.dump_ROOT_2018(self.data,self.root_filename,EXTTRIG,GRTRIG,dumppulse)

    def get_output_base(self):
        dirname = "{}/output/run{:04d}_n{:04d}".format(self.DATADIR, self.pulse_runnum,self.noise_runnum)
        if self.catecutname == "nocatecut":
            pass 
        else:
            dirname += "_"
    ...
    def savetxt_linefit_each(self,linename):
        ...
        if not self.linename in self.linename_list:
            ...
            sys.exit()
        self.each_csv = self.get_output_base()+"run{:04d}_n{:04d}".format(self.pulse_runnum,self.noise_runnum)+ "_%s.csv"%self.linename
...
#            fout.write("%d,%d,%d,%d,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f\n"%(self.pulse_runnum,self.cut_pre,self.cut_post,self.data.n_good_channels(),reson,tail_frac_on,resoncut,tail_frac_oncut,resoff,tail_frac_off))
#            fout.close()
#
#        plt.title("{}, {} good chans\n{}".format(self.spill.runstr,self.data.n_good_channels(),linename))
#
...
#        with PdfPages(self.get_output_base()+"run{:04d}_n{:04d}".format(self.pulse_runnum,self.noise_runnum)+ "_%s_nobeam.pdf"%linename) as pdf:
...
#            fig.tight_layout()
#            # fig.suptitle("run{:04d} %s (catecut%s)".format(self.pulse_runnum,linename,str(catecut)), fontsize=20)
...
#        self.linefit_each_dict = {}
#        print "printing each spectra of %s starts ...."%(linename)
#        with PdfPages(self.get_output_base()+"run{:04d}_n{:04d}".format(self.pulse_runnum,self.noise_runnum)+ "_%s.pdf"%linename) as pdf:
#            fig = plt.figure(figsize=(20,15))
#            coltmp=ds0.column_number
#            for ds in self.data:
#                col = ds.column_number
...
#        resol_file = self.get_output_base()+"run{:04d}_n{:04d}".format(self.pulse_runnum,self.noise_runnum)+ "_%s.csv"%(linename)
...
#        ax.set_title("run{:04d}, {} good chans\n {}".format(self.pulse_runnum,self.data.n_good_channels(),linename))
#
#        if not beam:
...
#            plt.savefig(self.get_output_base()+"run{:04d}_n{:04d}".format(self.pulse_runnum,self.noise_runnum)+ "_resolhist_%s_nobeam.png"%linename)
#        else:
#            plt.figtext(0.65,0.95,"on_resol mean =%.3f [eV]"%(self.resol1_mean))
#            plt.figtext(0.65,0.91,"off_resol mean =%.3f [eV]"%(self.resol2_mean))
#            plt.savefig(self.get_output_base()+"run{:04d}_n{:04d}".format(self.pulse_runnum,self.noise_runnum)+ "_resolhist_%s.png"%linename)
#        plt.close()
#
...
#                figname = self.get_output_base()+"run{:04d}".format(self.pulse_runnum)+ "_grppulse_%s_Ecut_%s_%s_%s_to_%s.pdf"%(str(chan),str(self.energy_cut),self.pulse_cut_category, str(self.cutMin), str(self.cutMax))
#                pagetitle = "run%04d Chan%d w/ %s cut (%s - %s) energy_cut = %s (%.2f - %.2f)" %(self.pulse_runnum, chan,self.pulse_cut_category, str(self.cutMin), str(self.cutMax), 
#                                                  str(self.energy_cut), float(self.energyMin), float(self.energyMax))                                     
#            elif self.energy_cut == True and self.cut_category == "None":
#                figname = self.get_output_base()+"run{:04d}".format(self.pulse_runnum)+ "_grppulse_%s_Ecut_%s_to_%s.pdf"%(str(chan),str(self.energy_cut), str(self.energyMin), str(self.energyMax))
#                pagetitle = "run%04d Chan%d w/o any cuts energy_cut = %s (%.2f - %.2f)"%(self.pulse_runnum,chan,str(self.energy_cut), float(self.energyMin), float(self.energyMax))       
#            elif self.energy_cut == False and self.cut_category != "None":
#                figname = self.get_output_base()+"run{:04d}".format(self.pulse_runnum)+ "_grppulse_%s_Ecut_%s_%s_%s_to_%s.pdf"%(str(chan),str(self.energy_cut),self.pulse_cut_category, str(self.cutMin), str(self.cutMax))
#                pagetitle = "run%04d Chan%d w/ %s cut (%s - %s) energy_cut = %s "%(self.pulse_runnum, chan,self.pulse_cut_category, str(self.cutMin), str(self.cutMax), str(self.energy_cut)) 
#            elif self.energy_cut == False and self.cut_category == "None":
#                figname = self.get_output_base()+"run{:04d}".format(self.pulse_runnum)+ "_grppulse_%s.pdf"%(str(chan))
#                pagetitle = "run%04d Chan%d w/o any cuts"%(self.pulse_runnum, chan)
#
...
#                                #plt.savefig(self.get_output_base()+"run{:04d}_n{:04d}".format(self.pulse_runnum,self.noise_runnum)+ "_grppulse_%s_%d.png"%(str(chan),ev))
#                                pdf.savefig()
#                                plt.clf()
#                    
...
#            grtchs.insert(0,chan)
#            grtchs = [x for x in grtchs if not x in self.badchans]
#            #figname = self.get_output_base()+"run{:04d}".format(self.pulse_runnum)+ "_grpPVmap_%s_Ecut_%s_%s_%s_to_%s.pdf"%(str(chan),str(self.energy_cut),self.cut_category, str(self.cutMin), str(self.cutMax))
...
#                    plt.title("run{:04d} peak value of each pixels\nprimary pulse peak value = {:.1f}".format(self.pulse_runnum,float(peak_value_all[i][j][chan])))
#                    pdf.savefig()
#                    plt.clf()
#            print "fig saved at ",figname
#            plt.close()
