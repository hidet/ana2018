...
2019-05-21 ; ver 1.5; timing analysis bug fixed, and modified the analysis processes by HT
2019-06-07 ; ver 1.6; HT added multiple runs, use_new_filter, vary_tail, and row decimals
    ...
    def ana(self,forceNew=False,summaryNew=False,calibNew=False,
        ...
        if self.extall:
            try:
                ...
                if self.spill is not None: self.beamflag_analysis(forceNew=exttrigNew)
            except:
        ...
        for ds in self.data:
            ...
            if self.spill is not None:
                ext.define_beam_timing(ds,self.external_trigger_rowcount[:],
                                       self.spill.typical_external_trigger_spacing,forceNew=forceNew)
