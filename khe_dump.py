""" khe_dump.py is a function package to dumping into a ROOT file for HEATES E62 

History: 
2018-08-16 ; ver 1.0; branched from khe_ana.py, T.H.
...
def dump_ROOT_2018(data,fout,EXTTRIG=True, GRTRIG=True, dumppulse=False):
    ...
    bp_rowmodn         = np.zeros(1,dtype=np.float64)
    # --- cut paramaters ---
    ...
    if EXTTRIG:
        ...
        bp_rows_after_last_spill_start = np.zeros(1, dtype=np.int64)
        bp_rows_until_next_spill_start = np.zeros(1, dtype=np.int64)
    ...
    ptbranch('rowmodn',        bp_rowmodn,         'rowmodifiedn/D')
    # --- cut paramaters 
    ...
    if EXTTRIG:
        ...
        ptbranch('row_after_spill', bp_rows_after_last_spill_start, 'row_after_spill/L')
        ptbranch('row_next_spill',  bp_rows_until_next_spill_start, 'row_next_spill/L')
    if GRTRIG:
    ...
    for ds in data:
        ...
        #np_energy = ds.calibration[attr].ph2energy(np_filt_value)
        ...
        np_rowmodn       =np_rowcount-(row_adjust*util.NUM_ROWS)-util.GLOBAL_PT_OFFSET
        ...
        if EXTTRIG:
            ...
            np_rows_after_last_spill_start         =np.array(ds.rows_after_last_spill_start)
            np_rows_until_next_spill_start         =np.array(ds.rows_until_next_spill_start)
        ...
        for i in xrange(ds.nPulses):
            ...
            if EXTTRIG:
                ...
                bp_rows_after_last_spill_start[0] = np_rows_after_last_spill_start[i]
                bp_rows_until_next_spill_start[0] = np_rows_until_next_spill_start[i]
            ptfill()
