...
2019-05-21 ; ver 1.3; HT minor updated
"""
...
def dump_ROOT(data,fout="hoge.root",EXTTRIG=True, GRTRIG=True, dumppulse=False):
    ...
    if EXTTRIG and GRTRIG:    
        ...
        htime_khe_name="htime_khe%d"%(run)
        htime_khe = ROOT.TH1F(htime_khe_name,htime_khe_name,nbin,minx,maxx)
        htime_khe.GetXaxis().SetTitle("Time (1ch=240ns)")
        htime_khe.GetYaxis().SetTitle("Counts / ch")
        htime_khe.Sumw2()
    # ---------------------------------------------------------------------
    ...
    for ds in data:
        ...
        if EXTTRIG and GRTRIG:
            ...
            set_hist1d(htime_khe,np_dt,enecut2_ind)
        # --------------------------------------------------------------------------
        cdhist.cd()
        nbin = 25000
        minx = 0.
