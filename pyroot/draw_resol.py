...
for t,fname,rlc,hoff,hon in zip(trees,fnames,rlcuts,hene_offs,hene_ons):
    ...
    if (os.path.isfile(ftmpname) is False):
        ...
        print "%s in %s has been read."%(treename,ftmpname)
    chtmp=0
    nch=0
    for e in newt:
        ch=e.ch
        if chtmp<ch:# ch is changed
            ...
            print "ch%d"%(ch)
            chtmp=ch
        if e.beam==1:# spill on
            hon.Fill(e.energy)
        elif e.beam==0:# spill off
            hoff.Fill(e.energy)
    linenames=["CrKAlpha","CoKAlpha","CuKAlpha"]
    for linename in linenames:
        plot_linefit(linename,hoff,hon,title="run0397 %d pixels %s %s %s"%(nch,rlc,template,linename),savename=fname+"_"+linename)
